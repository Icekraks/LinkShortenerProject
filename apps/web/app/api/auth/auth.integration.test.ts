...
describeIfIntegration("Auth integration", () => {
  ...
  beforeAll(async () => {
    ...
    // Tables created by migrations, just ensure they exist
    await pool.query(`
  ...
  beforeEach(async () => {
    ...
    // Truncate in dependency order (sessions before users)
    await pool.query("TRUNCATE TABLE sessions, user_credentials, users RESTART IDENTITY")
    vi.resetModules()
  })
